...
type ContextValue<ItemElement extends HTMLElement, ItemData extends BaseItemData> = {
...
function createCollection<ItemElement extends HTMLElement, ItemData extends BaseItemData = BaseItemData>(name: string) {
  ...
  const [CollectionContextProvider, useCollectionContext] = createCollectionContext<
    ContextValue<ItemElement, ItemData>
  >(PROVIDER_NAME, {
  ...
  const CollectionProviderImpl: React.FC<{
    ...
    const { children, scope, state } = props;
    ...
    return (
      <CollectionContextProvider
        ...
        collectionRef={composeRefs as React.Ref<CollectionElement | null>}
  ...
  const CollectionSlot = React.forwardRef<CollectionElement, CollectionProps>((props, forwardedRef) => {
    ...
    const composedRefs = useComposedRefs(
      forwardedRef,
      context.collectionRef as unknown as React.ForwardedRef<HTMLElement>
    );
  ...
  const CollectionItemSlot = React.forwardRef<ItemElement, CollectionItemSlotProps>((props, forwardedRef) => {
    ...
    React.useEffect(() => {
      const oldItemData = memoizedItemData;
      setItemMap(map => {
        if (!element) {
          return map;
        }
        ...
        if (!map.has(element)) {
          map.set(element, { ...(oldItemData as unknown as ItemData), element });
          return map.toSorted(sortByDocumentPosition);
        }

        return map.set(element, { ...(oldItemData as unknown as ItemData), element }).toSorted(sortByDocumentPosition);
      });
  ...
  return [{ ItemSlot: CollectionItemSlot, Provider: CollectionProvider, Slot: CollectionSlot }, functions] as const;
}
