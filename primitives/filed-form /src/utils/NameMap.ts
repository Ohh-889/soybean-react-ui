import type { PathTuple } from './util';
 ...
 */
function normalize(namePath: PathTuple): string {
  return (
    namePath
...
class NameMap<T> {
  ...
  public set(key: PathTuple, value: T) {
    this.kvs.set(normalize(key), value);
  }

  public get(key: PathTuple) {
    return this.kvs.get(normalize(key));
  }

  public update(key: PathTuple, updater: (origin: T) => T | null) {
    const origin = this.get(key);

    const next = updater(origin as T);
